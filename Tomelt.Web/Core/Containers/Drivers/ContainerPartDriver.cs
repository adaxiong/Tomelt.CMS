...
using Tomelt.Core.Containers.ViewModels;
...
namespace Tomelt.Core.Containers.Drivers {
    public class ContainerPartDriver : ContentPartDriver<ContainerPart> {
        ...
        public ContainerPartDriver(
            ...
            _contentManager = tomeltServices.ContentManager;
        ...
        protected override DriverResult Editor(ContainerPart part, dynamic shapeHelper) {
            if (!_contentDefinitionManager.ListTypeDefinitions().Any(typeDefinition => typeDefinition.Parts.Any(partDefinition => partDefinition.PartDefinition.Name == "ContainablePart"))) {
                _tomeltServices.Notifier.Warning(T("There are no content types in the system with a Containable part attached. Consider adding a Containable part to some content type, existing or new, in order to relate items to this (Container enabled) item."));
            }
