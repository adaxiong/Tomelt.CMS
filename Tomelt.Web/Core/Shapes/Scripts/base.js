(function ($) {
    ...
    $.extend({
        tomelt: {
            __cookieName: "Tomelt", // Tomelt, on a diet
            __cookieExpiration: 180, // roughly 6 months
            ___doCookiePathHack: null,
            __doCookiePathHack: function (path) {
                if ($.tomelt.___doCookiePathHack != null) {
                    return $.tomelt.___doCookiePathHack;
                }
                var testCookie = "__tomelt__";
                $.cookie(testCookie, "1", { path: path });
                $.tomelt.___doCookiePathHack = !$.cookie(testCookie);
                $.cookie(testCookie, null);
                return $.tomelt.___doCookiePathHack;
            },
            cookie: function (scope, value, options) { // a light-weight wrapper around $.cookie for an tomelt.* cookie name
                // because some browsers are weird about the path
                if (options && options.path && $.tomelt.__doCookiePathHack(options.path)) {
                    options.path = options.path.substring(0, options.path.lastIndexOf("/"));
                }
                return $.cookie($.tomelt.__cookieName + (scope ? scope.toLowerCase() : ""), value, options);
            },
            cookiesInTheTomelt: function () {
                return $.tomelt.cookiesLike($.tomelt.__cookieName);
            },
            ...
            setting: function (name, value, options) { // cookie-stored settings (only, at the moment)
                ...
                if (typeof value === "undefined") { // try to get the setting value from the default "root" cookie
                    var cookies = $.tomelt.cookiesInTheTomelt();
                    for (var i = 0; i < cookies.length; i++) {
                    ...
                    var cookie = $.tomelt.cookie(scope);
                    ...
                    $.tomelt.cookie(scope, dataString, { expires: $.tomelt.__cookieExpiration, path: (options && options.path) || "/" }); // todo: default path should be app path
                }
            }
        }
    });
