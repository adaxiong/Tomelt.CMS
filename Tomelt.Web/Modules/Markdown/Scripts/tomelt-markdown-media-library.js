    ...
    editors.each(function() {
        ...
        if (Boolean($(this).data("manage-media"))) {
            editor.hooks.set("insertImageDialog", function (callback) {
                ...
                var url = location.href.substr(0, adminIndex) + "/Admin/Tomelt.MediaLibrary?dialog=true";
                $.colorbox({
                    href: url,
                    iframe: true,
                    reposition: true,
                    ...
                    onClosed: function () {
                        ...
                        for (var i = 0; i < selectedData.length; i++) {
                            var renderMedia = location.href.substr(0, adminIndex) + "/Admin/Tomelt.MediaLibrary/MediaItem/" + selectedData[i].id + "?displayType=Raw";
                            $.ajax({
                                async: false,
                                type: 'GET',
                                url: renderMedia,
