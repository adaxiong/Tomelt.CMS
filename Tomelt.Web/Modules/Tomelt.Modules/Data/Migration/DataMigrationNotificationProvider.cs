...
namespace Tomelt.Modules.Data.Migration {
    /// <summary>
    ...
    /// </summary>
    [TomeltSuppressDependency("Tomelt.Data.Migration.DataMigrationNotificationProvider")]
            ...
            if (features.Any()) {
                ...
                yield return new NotifyEntry { Message = T("Some features need to be upgraded: {0}", 
                    T(string.Join(", ", features
                        .Select(feature =>
                            string.Format("<a href=\"{0}#{1}\">{2}</a>", urlHelper.Action("Features", "Admin", new RouteValueDictionary { { "area", "Tomelt.Modules" } }), feature.AsFeatureId(n => T(n)), feature))))),
