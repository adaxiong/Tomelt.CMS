...
using Tomelt.Packaging.Extensions;
    ...
    public class PackageInstaller : IPackageInstaller {
        ...
        protected PackageInfo InstallPackage(IPackage package, IPackageRepository packageRepository, string location, string applicationPath) {
            ...
            try {
                ...
                throw new TomeltException(T("Unable to backup existing local package directory."), exception);
            }
            ...
            if (previousInstalled) {
                ...
                try {
                    ...
                    throw new TomeltException(T("Unable to un-install local package before updating."), exception);
                }
            }

            var packageInfo = ExecuteInstall(package, packageRepository, location, applicationPath);

            // check the new package is compatible with current Tomelt version
            var descriptor = package.GetExtensionDescriptor(packageInfo.ExtensionType);

            if(descriptor != null) {
                if(new FlatPositionComparer().Compare(descriptor.TomeltVersion, typeof(ContentItem).Assembly.GetName().Version.ToString()) >= 0) {
                    if (previousInstalled) {
                    ...
                    Logger.Error(String.Format("The package is compatible with version {0} and above. Please update Tomelt or install another version of this package.", descriptor.TomeltVersion));
                    throw new TomeltException(T("The package is compatible with version {0} and above. Please update Tomelt or install another version of this package.", descriptor.TomeltVersion));
                }    
            }
        ...
        public void Uninstall(string packageId, string applicationPath) {
            ...
            if (string.IsNullOrEmpty(extensionFullPath) ||
                ...
                throw new TomeltException(T("Package not found: {0}", packageId));
            }
        ...
        private bool RestoreExtensionFolder(string extensionFolder, string extensionId) {
            ...
            if (source.Exists) {
                ...
                if (localTempPath == null) {
                    throw new TomeltException(T("Backup folder {0} has too many backups subfolder (limit is 1,000)", tempPath));
                }
