...
namespace Tomelt.Recipes.Providers.Builders {
    public class ContentStep : RecipeBuilderStep {
        ...
        public override void Build(BuildContext context) {
            ...
            var contentItems = dataContentTypes.Any()
                ? _tomeltServices.ContentManager.Query(exportVersionOptions, dataContentTypes.ToArray()).List().ToArray()
                : Enumerable.Empty<ContentItem>();

            if(schemaContentTypes.Any())
                context.RecipeDocument.Element("Tomelt").Add(ExportMetadata(schemaContentTypes));

            if(contentItems.Any())
                context.RecipeDocument.Element("Tomelt").Add(ExportData(dataContentTypes, contentItems));
        }
        ...
        private XElement ExportData(IEnumerable<string> contentTypes, IEnumerable<ContentItem> contentItems) {
            ...
            var orderedContentItemsQuery =
                ...
                let identity = _tomeltServices.ContentManager.GetItemMetadata(contentItem).Identity.ToString()
                orderby identity
                select contentItem;
        ...
        private XElement ExportContentItem(ContentItem contentItem) {
            return _tomeltServices.ContentManager.Export(contentItem);
        }
    }
}