...
namespace Tomelt.Users.Services {
    [TomeltSuppressDependency("Tomelt.Security.NullMembershipService")]
    public class MembershipService : IMembershipService {
        ...
        public IUser CreateUser(CreateUserParams createUserParams) {
            ...
            var user = _tomeltServices.ContentManager.New<UserPart>("User");
            ...
            _tomeltServices.ContentManager.Create(user);
        ...
        public IUser GetUser(string username) {
            ...
            return _tomeltServices.ContentManager.Query<UserPart, UserPartRecord>().Where(u => u.NormalizedUserName == lowerName).List().FirstOrDefault();
        }

        public IUser ValidateUser(string userNameOrEmail, string password) {
            var lowerName = userNameOrEmail == null ? "" : userNameOrEmail.ToLowerInvariant();

            var user = _tomeltServices.ContentManager.Query<UserPart, UserPartRecord>().Where(u => u.NormalizedUserName == lowerName).List().FirstOrDefault();

            if (user == null)
                user = _tomeltServices.ContentManager.Query<UserPart, UserPartRecord>().Where(u => u.Email == lowerName).List().FirstOrDefault();
        ...
        private bool ValidatePasswordHashed(UserPart userPart, string password) {
            ...
            if (isValid && userPart.HashAlgorithm != DefaultHashAlgorithm) {
                var keepOldConfiguration = _appConfigurationAccessor.GetConfiguration("Tomelt.Users.KeepOldPasswordHash");
