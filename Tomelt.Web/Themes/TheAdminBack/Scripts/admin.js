    ...
    $.fn.extend({
        expandoControl: function (getControllees, options) {
            ...
            _this.each(function (index, element) {
                ...
                if ((settings.remember && "closed" === $.tomelt.setting(__cookieName, { key: settings.key + "-" + controller.text(), path: settings.path }))
                    || settings.collapse
                    || (controller.closest("li").hasClass("collapsed") && !(settings.remember && "open" === $.tomelt.setting(__cookieName, { key: settings.key + "-" + controller.text(), path: settings.path })))) {
                ...
                glyph.click(function () {
                    ...
                    if (settings.remember && !settings.collapse) { // remembering closed state as true because that's not the default - doesn't make sense to remember if the controllees are always to be collapsed by default
                        ...
                        $.tomelt.setting(__cookieName, !__this.hasClass("closed") ? "closed" : "open", { key: settings.key + "-" + controller.text(), path: settings.path });
                    }
