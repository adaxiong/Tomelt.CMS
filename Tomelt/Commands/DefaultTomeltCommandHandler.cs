...
namespace Tomelt.Commands {
    public abstract class DefaultTomeltCommandHandler : ICommandHandler {
        protected DefaultTomeltCommandHandler() {
        ...
        private void SetSwitchValue(KeyValuePair<string, string> commandSwitch) {
            ...
            if (propertyInfo.GetCustomAttributes(typeof(TomeltSwitchAttribute), false).Length == 0) {
                throw new InvalidOperationException(T("A property \"{0}\" exists but is not decorated with \"{1}\"", commandSwitch.Key, typeof(TomeltSwitchAttribute).Name).Text);
            }
        ...
        private void CheckMethodForSwitches(MethodInfo methodInfo, IDictionary<string, string> switches) {
            ...
            foreach (TomeltSwitchesAttribute switchesAttribute in methodInfo.GetCustomAttributes(typeof(TomeltSwitchesAttribute), false)) {
