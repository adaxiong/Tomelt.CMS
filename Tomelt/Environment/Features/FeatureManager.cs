...
using Tomelt.Environment.Extensions.Helpers;
using Tomelt.Environment.Extensions.Models;
using Tomelt.Localization;
using Tomelt.Logging;

namespace Tomelt.Environment.Features {
                    ...
                    (currentFeatureId, featuresState) => {
                        ...
                        return feature.Key.Dependencies
                                      .Select(fId => {
                                          ...
                                          if (states.Count == 0) {
                                              throw new TomeltException(T("Failed to get state for feature {0}", fId));
                                          }

                                          if (states.Count > 1) {
                                              throw new TomeltException(T("Found {0} states for feature {1}", states.Count, fId));
                                          }
