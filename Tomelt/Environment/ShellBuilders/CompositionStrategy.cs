...
namespace Tomelt.Environment.ShellBuilders {
    public class CompositionStrategy : ICompositionStrategy {
        ...
        public ShellBlueprint Compose(ShellSettings settings, ShellDescriptor descriptor) {
            ...
            if (descriptor.Features.Any(feature => feature.Name == "Tomelt.Framework"))
        ...
        private static IEnumerable<string> GetExcludedTypes(IEnumerable<Feature> features) {
            ...
            foreach (var feature in features) {
                foreach (var type in feature.ExportedTypes) {
                    foreach (TomeltSuppressDependencyAttribute replacedType in type.GetCustomAttributes(typeof(TomeltSuppressDependencyAttribute), false)) {
        ...
        private static IEnumerable<Feature> BuiltinFeatures() {
            yield return new Feature {
                ...
                ExportedTypes =
                    typeof(TomeltStarter).Assembly.GetExportedTypes()
                    .Where(t => t.IsClass && !t.IsAbstract)
                    .Except(new[] { typeof(DefaultTomeltHost) })
                    .ToArray()
            };
        }
