...
namespace Tomelt.Logging {
    public class TomeltFileAppender : RollingFileAppender {
        /// <summary>
            ...
            lock (this) {
                ...
                for (int i = 1; !fileOpened && i <= Retries; i++) {
                    try {
                        ...
                        LogLog.Error(typeof(TomeltFileAppender), string.Format("TomeltFileAppender: Failed to open [{0}]. Attempting [{1}-{2}] instead.", fileName, fileName, newSuffix));
                    }
                }
