...
namespace Tomelt.Logging {
    [Serializable]
    public class TomeltLog4netLogger : MarshalByRefObject, Logger, IShim {
        private static readonly Type declaringType = typeof(TomeltLog4netLogger);
        ...
        internal TomeltLog4netLogger() {
        }

        internal TomeltLog4netLogger(ILog log, TomeltLog4netFactory factory)
        ...
        private ShellSettings LoadSettings() {
            ...
            var tomeltHost = HostContainer.Resolve<ITomeltHost>();
            if (tomeltHost == null)
                return null;

            var shellContext = tomeltHost.GetShellContext(shellSettings);
        ...
        protected internal TomeltLog4netFactory Factory { get; set; }
