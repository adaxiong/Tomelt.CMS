...
using Tomelt.ResponseFiles;

namespace Tomelt.Host {
    ...
    /// between the command line and the CommandHostAgent, which is known to the Tomelt
    ...
    public class CommandHost : MarshalByRefObject, IRegisteredObject {
        ...
        private static void ExtendLifeTimeLeases(TextReader input, TextWriter output) {
            // Tomelt objects passed as parameters into this AppDomain should derive from MarshalByRefObject and have
        ...
        public CommandReturnCodes RunCommand(TextReader input, TextWriter output, Logger logger, TomeltParameters args) {
            ExtendLifeTimeLeases(input, output);
        ...
        public CommandReturnCodes RunCommandInSession(TextReader input, TextWriter output, Logger logger, TomeltParameters args) {
            ExtendLifeTimeLeases(input, output);
        ...
        public CommandReturnCodes RunCommands(TextReader input, TextWriter output, Logger logger, IEnumerable<ResponseLine> responseLines) {
            ...
            foreach (var line in responseLines) {
                ...
                var args = new TomeltParametersParser().Parse(new CommandParametersParser().Parse(line.Args));
        ...
        private object CreateAgent() {
            return Activator.CreateInstance("Tomelt.Framework", "Tomelt.Commands.CommandHostAgent").Unwrap();
        }
