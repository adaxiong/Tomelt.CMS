...
namespace Tomelt.HostContext {
    public class CommandHostContextProvider : ICommandHostContextProvider {
        ...
        public void Shutdown(CommandHostContext context) {
            try {
                if (context.CommandHost != null) {
                    LogInfo(context, "Shutting down Tomelt session...");
        ...
        private void Initialize(CommandHostContext context) {
            context.Arguments = new TomeltParametersParser().Parse(new CommandParametersParser().Parse(_args));
            ...
            LogInfo(context, "Detecting tomelt installation root directory...");
            context.TomeltDirectory = GetTomeltDirectory(context.Arguments.WorkingDirectory);
            LogInfo(context, "Tomelt root directory: \"{0}\"", context.TomeltDirectory.FullName);

            LogInfo(context, "Creating ASP.NET AppDomain for command agent...");
            context.CommandHost = CreateWorkerAppDomainWithHost(context.Arguments.VirtualPath, context.TomeltDirectory.FullName, typeof(CommandHost));

            LogInfo(context, "Starting Tomelt session");
        ...
        private DirectoryInfo GetTomeltDirectory(string directory) {
            for (var directoryInfo = new DirectoryInfo(directory); directoryInfo != null; directoryInfo = directoryInfo.Parent) {
                ...
                var tomeltFrameworkFileInfo = new FileInfo(Path.Combine(binDirectoryInfo.FullName, "Tomelt.Framework.dll"));
                if (!tomeltFrameworkFileInfo.Exists)
                    continue;
            ...
            throw new ApplicationException(
                string.Format("Directory \"{0}\" doesn't seem to contain an Tomelt installation", new DirectoryInfo(directory).FullName));
        }
