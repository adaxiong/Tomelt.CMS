...
namespace Tomelt {
    class TomeltHost
    {
        ...
        public TomeltHost(TextReader input, TextWriter output, string[] args) {
            _input = input;
            _output = output;
        ...
        private CommandHostContext CommandHostContext() {
            _output.WriteLine("Initializing Tomelt session. (This might take a few seconds...)");
            ...
            if (result.StartSessionResult == result.RetryResult) {
                ...
                _output.WriteLine("Failed to initialize Tomelt session.");    
            }
        ...
        private CommandReturnCodes RunCommandInSession(CommandHostContext context, string command) {
            try {
                var args = new TomeltParametersParser().Parse(new CommandParametersParser().Parse(new CommandLineParser().Parse(command)));
                return context.CommandHost.RunCommandInSession(_input, _output, context.Logger, args);
            }
            catch (AppDomainUnloadedException) {
                _output.WriteLine("AppDomain of Tomelt session has been unloaded. (Retrying...)");
                return context.RetryResult;
            }
        }

        private void DisplayInteractiveHelp() {
            _output.WriteLine("The Tomelt command interpreter supports running a few built-in commands");
            _output.WriteLine("as well as specific commands from enabled features of an Tomelt installation.");
            _output.WriteLine("");
        ...
        private void DisplayUsageHelp() {
            _output.WriteLine("Executes Tomelt commands from a Tomelt installation directory.");
            _output.WriteLine("");
            _output.WriteLine("Usage:");
            _output.WriteLine("   tomelt.exe");
            _output.WriteLine("       Starts the Tomelt command interpreter");
            _output.WriteLine("");
            _output.WriteLine("   tomelt.exe <command-name> [arg1] ... [argn] [/switch1[:value1]] ... [/switchn[:valuen]]");
            _output.WriteLine("       Executes a single command");
            _output.WriteLine("");
            _output.WriteLine("   tomelt.exe @response-file1 ... [@response-filen] [/switch1[:value1]] ... [/switchn[:valuen]]");
            ...
            _output.WriteLine("       Specifies the tomelt installation directory. The current directory is the default.");
            _output.WriteLine("");
            _output.WriteLine("   /Verbose");
            _output.WriteLine("   /v");
